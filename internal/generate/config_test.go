...
func TestSingleRunnerConfig_TLSCAFile(t *testing.T) {
	...
	cfg, _, err := SingleRunnerConfig(base, tokens)
	if err != nil {
	...
	cfg, _, err = SingleRunnerConfig(withCA, tokens)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMultiRunnerConfig_TLSCAFile(t *testing.T) {
	...
	cfg, _, err := MultiRunnerConfig(base, tokens)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if strings.Contains(cfg, "tls-ca-file") {
	...
	cfg, _, err = MultiRunnerConfig(withCA, tokens)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
