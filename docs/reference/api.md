...
| `runner_env` _[EnvVar](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.36/#envvar-v1-core) array_ | RunnerEnv sets extra env vars on the runner manager container, e.g. an<br />outbound HTTP_PROXY. Distinct from Environment, which only reaches job<br />builds via config.toml. Prefer valueFrom for secrets, plain values log<br />verbatim; a fieldRef needs an explicit apiVersion or reconciles forever. |  | Optional: \{\} <br /> |
